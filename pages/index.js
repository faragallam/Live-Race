...
export default function TeacherControl() {
  ...
  const handleFileUpload = (e) => {
    if (e.target.files[0]) {
      alert("Questions loaded! The game is ready.");
    }
  };

  const startGame = () => {
    // This correctly routes you to the race track with your settings
      ...
      <div className="max-w-2xl mx-auto bg-white p-8 rounded-2xl shadow-xl border-4 border-blue-200">
        ...
        <div className="mb-8 p-6 bg-green-50 rounded-xl border-2 border-green-200">
          ...
          <select value={mode} onChange={(e) => setMode(e.target.value)} className="w-full p-3 rounded-lg border-2 border-gray-300 text-lg cursor-pointer">
            <option value="race">Race to the Finish</option>
            <option value="time">Time-Attack</option>
          </select>
        </div>

        <button onClick={startGame} className="w-full bg-blue-500 hover:bg-blue-600 text-white font-black text-2xl py-4 rounded-xl shadow-lg">
          LAUNCH GAME!
        </button>
      </div>
    </div>
